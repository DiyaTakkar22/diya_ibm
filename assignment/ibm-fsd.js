...
const findAllEmployees = () => {
  const url='http://localhost:9090/emp/get-all-emps';
  fetch(url)
    .then((res) => res.json())
...
const printAllData = (arr) => {
  for (let i = 0; i < arr.length; i++) {
    ...
    employeeInfo.textContent = `${i + 1}    
       First Name: ${ arr[i].firstName }     
         Salary: ${arr[i].salary}    
     Email: ${arr[i].email}`;
    document.getElementById("employeeData").appendChild(employeeInfo);
  }
};
...
const findEmployeeByName = () => {
    const firstName = document.getElementById("searchByNameInput").value;
...
const deleteEmployee = () => {
  ...
  fetch("http://localhost:9090/emp/delete-emp/" +delEmpId, {
    method: "DELETE"
  })
    .then((res) => {
      console.log("Deleted");
